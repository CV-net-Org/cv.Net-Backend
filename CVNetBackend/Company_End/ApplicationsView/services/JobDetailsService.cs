...
public class JobDetailsService
{
    ...
    public async Task<FullApplicantProfileDto?> GetFullApplicantProfileAsync(string appId)
    {
        ...
        const string sql = @"
            ...
            -- [2] Experience
            SELECT * FROM public.snapshot_experience
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [3] Education
            SELECT * FROM public.snapshot_education
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [4] Skills
            SELECT * FROM public.snapshot_skills
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [5] Projects
            SELECT * FROM public.snapshot_projects
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [6] Publications
            SELECT * FROM public.snapshot_publications
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [7] Certifications
            SELECT * FROM public.snapshot_certifications
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [8] Memberships
            SELECT * FROM public.snapshot_memberships
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [9] Languages
            SELECT * FROM public.snapshot_languages
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [10] Teaching Experience
            SELECT * FROM public.snapshot_teaching_experience
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [11] Research Experience
            SELECT * FROM public.snapshot_research_experience
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [12] Awards
            SELECT * FROM public.snapshot_awards
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [13] Volunteers
            SELECT * FROM public.snapshot_volunteers
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);

            -- [14] Social Links
            SELECT * FROM public.snapshot_social_links
            WHERE snapshot_id = (SELECT snapshot_id FROM public.job_applications WHERE id = @appId::uuid);
        ";
            ...
            foreach (var app in pendingApps)
            {
                await conn.ExecuteAsync(
                    @"INSERT INTO public.reject_records (id, user_id, application_id, job_id, reason, rejected_date)
                      VALUES (uuid_generate_v4(), @uid::uuid, @aid::uuid, @jobId::uuid, 'Job Closed', CURRENT_TIMESTAMP)",
        ...
        try
        {
            ...
            await conn.ExecuteAsync(
                @"INSERT INTO public.call_for_interviews (id, user_id, application_id, job_id, interview_date, message, created_at)
                  VALUES (uuid_generate_v4(), @uid::uuid, @aid::uuid, @jid::uuid, @date, @msg, CURRENT_TIMESTAMP)",
    ...
    public async Task<bool> RejectApplicantAsync(string appId, RejectRequestDto dto)
    {
        ...
        try
        {
            ...
            await conn.ExecuteAsync(
                @"INSERT INTO public.reject_records (id, user_id, application_id, job_id, reason, rejected_date)
                  VALUES (uuid_generate_v4(), @uid::uuid, @aid::uuid, @jid::uuid, @reason, CURRENT_TIMESTAMP)",
