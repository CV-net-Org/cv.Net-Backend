...
public class UserProfileController : ControllerBase
{
    ...
    public async Task<IActionResult> GetFullProfile([FromQuery] string userId)
    {
        ...
        if (userRow == null) return NotFound(new { error = "User not found." });
        ...
        // If active profile is empty, default to Master
        ...
        // Mark Master Profile vs Target Roles
    ...
    // ✅ BRAND NEW ENDPOINT: Master Data Cloner
    // ✅ UPDATED ENDPOINT: Smart Merge Master Data Cloner (No Replacements)
    ...
    public async Task<IActionResult> CloneProfile([FromBody] CloneProfileDto data)
    {
        ...
        // 1. Merge text fields ONLY if the target field is currently empty or null.
        // COALESCE(NULLIF(t.field, ''), m.field) means: "If target is empty, use master. Otherwise, keep target."
        await conn.ExecuteAsync(@"
        ...
        // 2. Append Arrays safely: INSERT only if an identical record doesn't already exist in the target profile.
        string[] queries = {
            // SKILLS: Only add if the skill name doesn't already exist
            ...
            // EXPERIENCE: Only add if company and role aren't already listed
            ...
            // EDUCATION: Only add if degree and organization aren't already listed
            ...
            // SOCIAL LINKS: Only add if the platform isn't already listed
    ...
    {
        var dict = JsonSerializer.Deserialize<Dictionary<string, string>>(payload.GetRawText());
        if (dict == null || !dict.ContainsKey("profileId")) return BadRequest("Missing Profile ID.");

        string profileId = dict["profileId"];
        dict.Remove("profileId"); dict.Remove("id"); dict.Remove("profile_id"); dict.Remove("created_at"); dict.Remove("updated_at");

        var columns = new List<string> { "id", "profile_id", "created_at", "updated_at" };
        var values = new List<string> { "gen_random_uuid()", "@profileId", "CURRENT_TIMESTAMP", "CURRENT_TIMESTAMP" };
        var parameters = new DynamicParameters();
        if(!Guid.TryParse(profileId, out Guid parsedGuid)) return BadRequest("Invalid Profile ID");
        ...
        foreach (var kvp in dict)
        {
            columns.Add(kvp.Key);
            values.Add($"@{kvp.Key}");
            parameters.Add(kvp.Key, string.IsNullOrEmpty(kvp.Value) ? null : kvp.Value);
        }

        using var conn = new NpgsqlConnection(_connString);
        await conn.OpenAsync();
        try {
            await conn.ExecuteAsync($"INSERT INTO public.\"{tableName.ToLower()}\" ({string.Join(", ", columns)}) VALUES ({string.Join(", ", values)})", parameters);
            return Ok(new { success = true });
        } catch (PostgresException ex) when (ex.SqlState == "23503") { return BadRequest("Foreign Key Error: Active Profile not found."); }
    }
